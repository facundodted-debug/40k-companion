...
export function MatchupConfigScreen({ onBack, onAnalyze }: MatchupConfigScreenProps) {
  ...
  const canAnalyze = !!ownList && !!selectedSubfaction;
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="p-4" style={{ borderTop: '1px solid rgba(255,255,255,0.07)' }}>
        ...
        <button
          onClick={() => {
            if (!canAnalyze || !selectedSubfaction || !selectedCategory) return;
            onAnalyze(
              { name: selectedSubfaction.name, color: selectedCategory.color, abbr: selectedSubfaction.name.slice(0, 3).toUpperCase() },
