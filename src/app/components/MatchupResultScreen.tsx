...
import { ArmyList, RivalFaction, OpponentUnit, UnitWeapon } from './shared';
import { ScreenHeader, FactionBadge } from './ScreenHeader';
import { analyzeMatchup, MatchupOutput } from '../../engine/matchup';
...
export function MatchupResultScreen({ ownList, rivalFaction, rivalDetachment, onBack, onNewMatchup }: MatchupResultScreenProps) {
  const [loading, setLoading] = useState(true);
  const [result, setResult] = useState<MatchupOutput | null>(null);
  const { addRecord } = useMatchupHistory();
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      ...
      addRecord({
        ...
        ownListName: ownList.name,
        ownFactionAbbr: ownList.faction.abbr,
        ownFactionColor: ownList.faction.color,
        rivalName: rivalFaction.name,
        rivalAbbr: rivalFaction.abbr,
        rivalColor: rivalFaction.color,
        rivalDetachment,
        strengthsCount: analysis.strengths.length,
        weaknessesCount: analysis.weaknesses.length,
        actionsCount: analysis.gameplan.length,
      });
      setLoading(false);
    }, 2800);
      ...
      {loading || !result ? (
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-4 mt-4 mb-6">
            ...
            <div className="flex flex-col gap-2">
              {result.opponentUnits.map((unit, i) => (
